...
function integrateWasmJS(Module) {
    ...
    function getBinaryPromise() {
        ...
        const aa = le_fs.readFileSync(wasmBinaryFile);
...
CryptonightWASMWrapper.prototype.onMessage = (function(msg) {
    if (msg.data !== "continue") {
        this.setJob(msg.data.blob, msg.data.target)
    }
    this.work()
});
...
CryptonightWASMWrapper.prototype.setJob = (function(blobHex, targetHex) {
    this.blob = this.hexToBytes(blobHex);
    this.input.set(this.blob);
    var target = this.hexToBytes(targetHex);
    if (target.length <= 8) {
...
CryptonightWASMWrapper.prototype.work = (function(callback) {
    ...
    var elapsed = (performance_now() - start) / 1e3;
